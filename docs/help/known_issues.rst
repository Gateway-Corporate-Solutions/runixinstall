...
Some issues are out of the `runixinstall`_ projects scope, and the ones we know of are listed below.
...
.. note::
   ...
   If you know your time is correct on the machine, you can run ``runixinstall --skip-ntp`` to ignore time sync.
...
.. note::
   ...
   If you know the ISO is the latest, and that you have valid GPG keys, try ``runixinstall --skip-wkd`` to ignore waiting for the sync.

   If you skip WKD sync, you might end up with:

   .. code-block:: console

         > error: runixinstall: signature from "Anton Hvornum (Torxed) <torxed@archlinux.org>" is unknown trust
         > :: File /var/cache/pacman/pkg/runixinstall-1.2.3-4-x86_64.pkg.tar.xz is corrupted (invalid or corrupted package (PGP signature)).
...
Subsequently the ``runixinstall`` run might operate on a old keyring despite there being an update service for this.

There is really no way to reliably over time work around this issue in ``runixinstall``.
...
`AUR is unsupported <https://wiki.archlinux.org/title/Arch_User_Repository#Updating_packages>`_, and until that changes we cannot use AUR packages to solve feature requests in ``runixinstall``.
...
.. note::
   ...
   But in spirit of giving the community options, ``runixinstall`` supports :ref:`runixinstall.Plugins`, which means you can run ``runixinstall --plugin <url>`` and source an AUR plugin.

   `torxed/runixinstall-aur <https://github.com/torxed/runixinstall-aur>`_ is a reference implementation for plugins:

   .. code-block:: console

      # runixinstall --plugin https://archlinux.life/aur-plugin

   `phisch/runixinstall-aur <https://github.com/phisch/runixinstall-aur>`_ is another alternative:

   .. code-block:: console

      # runixinstall --plugin https://raw.githubusercontent.com/phisch/runixinstall-aur/master/runixinstall-aur.py

   .. warning::

      This will allow for unsupported usage of AUR during installation.

.. _#1686: https://github.com/archlinux/runixinstall/issues/1686
.. _#2002: https://github.com/archlinux/runixinstall/issues/2002
.. _#2144: https://github.com/archlinux/runixinstall/issues/2144
.. _#2185: https://github.com/archlinux/runixinstall/issues/2185
.. _#2213: https://github.com/archlinux/runixinstall/issues/2213
.. _#2679: https://github.com/archlinux/runixinstall/issues/2679
...
.. _runixinstall: https://github.com/archlinux/runixinstall/
.. _timedatectl show: https://github.com/archlinux/runixinstall/blob/e6344f93f7e476d05bbcd642f2ed91fdde545870/runixinstall/lib/installer.py#L136
