...
from runixinstall.lib.args import ArchConfigHandler
...
def test_user_config_roundtrip(
	...
	monkeypatch.setattr('sys.argv', ['runixinstall', '--config', str(config_fixture)])

	handler = ArchConfigHandler()
	arch_config = handler.config

	# the version is retrieved dynamically from an installed runixinstall package
...
def test_creds_roundtrip(
	...
	monkeypatch.setattr('sys.argv', ['runixinstall', '--creds', str(creds_fixture)])
