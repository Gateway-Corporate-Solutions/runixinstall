<!-- <div align="center"> -->
<img src="https://github.com/archlinux/runixinstall/raw/master/docs/logo.png" alt="drawing" width="200"/>
...
`runixinstall` can be run with a JSON configuration file. There are 2 different configuration files to consider,
...
[configuration file](https://github.com/archlinux/runixinstall/blob/master/examples/config-sample.json)
and an example of the credentials configuration here
[credentials file](https://github.com/archlinux/runixinstall/blob/master/examples/creds-sample.json).

**HINT:** The configuration files can be auto-generated by starting `runixinstall`, configuring all desired menu
points and then going to `Save configuration`.

To load the configuration file into `runixinstall` run the following command
```shell
runixinstall --config <path to user config file or URL> --creds <path to user credentials config file or URL>
```
...
However, when selecting to save configuration files, `runixinstall` will prompt for the option to encrypt the `user_credentials.json` file content.
...
* Store the encryption key in the environment variable `runixinstall_CREDS_DECRYPTION_KEY` which will be read automatically
...
* Attach the `/var/log/runixinstall/install.log` to the issue ticket. This helps us help you!
...
runixinstall is available in different languages which have been contributed and are maintained by the community.
...
to get started please follow [the guide](https://github.com/archlinux/runixinstall/blob/master/runixinstall/locales/README.md)
...
[interactive_installation.py](https://github.com/archlinux/runixinstall/blob/master/runixinstall/scripts/guided.py)
...
[full_automated_installation.py](https://github.com/archlinux/runixinstall/blob/master/examples/full_automated_installation.py)

# Profiles

`runixinstall` comes with a set of pre-configured profiles available for selection during the installation process.

- [Desktop](https://github.com/archlinux/runixinstall/tree/master/runixinstall/default_profiles/desktops)
- [Server](https://github.com/archlinux/runixinstall/tree/master/runixinstall/default_profiles/servers)

The profiles' definitions and the packages they will install can be directly viewed in the menu, or
[default profiles](https://github.com/archlinux/runixinstall/tree/master/runixinstall/default_profiles)
...
replace the runixinstall version with a newer one and execute the subsequent steps defined below.
...
3. Uninstall the previous version of runixinstall with `pip uninstall --break-system-packages runixinstall`
4. Now clone the latest repository with `git clone https://github.com/archlinux/runixinstall`
5. Enter the repository with `cd runixinstall`
...
6. To run the source code, there are 2 different options:
   - Run a specific branch version from source directly using `python -m runixinstall`, in most cases this will work just fine, the
      rare case it will not work is if the source has introduced any new dependencies that are not installed yet
   - Installing the branch version with `pip install --break-system-packages .` and `runixinstall`
...
This can be done by installing `pacman -S arch-install-scripts util-linux` locally and doing the following:
    ...
    # pip install --upgrade runixinstall
    # python -m runixinstall --script guided
...
`runixinstall` is installed and executed in [guided mode](#docs-todo). Once the installation is complete, ~~you can use qemu/kvm to boot the test media.~~<br>
...
There's also a [Building and Testing](https://github.com/archlinux/runixinstall/wiki/Building-and-Testing) guide.<br>
...
## Keyring out-of-date
For a description of the problem see https://runixinstall.archlinux.page/help/known_issues.html#keyring-is-out-of-date-2213 and discussion in issue https://github.com/archlinux/runixinstall/issues/2213.
...
To install Arch Linux alongside an existing Windows installation using  `runixinstall`, follow these steps:

1. Ensure some unallocated space is available for the Linux installation after the Windows installation.
2. Boot into the ISO and run `runixinstall`.
...
runixinstall promises to ship a [guided installer](https://github.com/archlinux/runixinstall/blob/master/runixinstall/scripts/guided.py) that follows
...
runixinstall primarily functions as a flexible library for managing services, packages, and other elements within an Arch Linux system.
This core library is the backbone for the guided installer that runixinstall provides. It is also designed to be used by those who wish to script their own custom installations.

Therefore, runixinstall will try its best to not introduce any breaking changes except for major releases which may break backward compatibility after notifying about such changes.


# Contributing

Please see [CONTRIBUTING.md](https://github.com/archlinux/runixinstall/blob/master/CONTRIBUTING.md)
